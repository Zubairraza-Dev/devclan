...
import UxDesignSvg from "@/components/SvgComponents/UxDesignSvg";
import LeftArrowSvg from "@/components/SvgComponents/LeftArrowSvg";
...
const ServiceDetailCard: React.FC<ServiceDetailCardProp> = ({
  ...
  const selectedData = portfolio?.slice(
    currentIndex,
    ...
    <div className="sm:w-full w-[93%] mx-auto">
      ...
      <div className="flex justify-between items-center w-full sm:mb-36 mb-20 sm:mt-5 flex-wrap ">
        ...
        <Image
          alt="ai image"
          src={image}
          className="w-full sm:w-[40%] sm:mt-0 mt-10"
        />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap 2xl:gap-20 xl:gap-12 sm:gap-10 text-customDark tracking-tighter sm:mt-12 ">
          {detailServices?.map((e) => (
            <div
              key={e.id}
              className="sm:w-[424px] sm:h-[352px] relative group sm:mt-0 mt-20"
            >
              <div className="absolute right-0 left-0 mt-3 sm:group-hover:scale-50 transition duration-700 ease-in-out origin-top-right scale-50 sm:scale-100">
                <Image
                  src={circle1}
                  alt="circle"
                  className="absolute mx-auto left-48 right-0"
                />
                <Image
                  src={circle2}
                  alt="circle"
              ...
              <div className="w-[95%] mx-auto  flex gap-4 flex-col h-full justify-center">
                ...
                <div className="text-[24px] font-bold">{e.title}</div>
      ...
      {(title === "Web App Development" ||
        ...
        <div className="sm:mt-32 my-12 sm:mb-44 flex flex-col gap-9 sm:gap-24 w-full relative overflow-hidden">
            ...
            {developmentProcess?.map((e, i) => (
              <div>
                <div className="w-full mb-6">
                  <div className="h-[2px] w-full absolute bg-[#EEEEEE] mt-[11px]"></div>

                  <div
                    className={`w-[24px] h-[24px] flex justify-center items-center border rounded-full relative ${
                      currentStep === i
                        ? " border-customBlue"
                        : " border-[#EEEEEE] "
                    }`}
                  >
                    <div
                      className={`w-[12px] h-[12px]${
                        currentStep === i ? " bg-customBlue" : " bg-[#EEEEEE] "
                      } rounded-full`}
                    ></div>
                  </div>
                </div>
                ...
                <div
                  key={i}
                  className={`h-[59px] w-auto px-5 tracking-tighter  shadow-lg sm:shadow-[0_1px_6px_rgba(0,0,0,0.5)]  font-semibold text-[16px] rounded-[10px]  flex flex-col box-border pt-2 hover:cursor-pointer text-nowrap
                      ${
                        currentStep === i
                          ? " bg-customBlue text-white "
                          : " text-customDark bg-white bg-opacity-5"
                      }`}
          ...
          {currentStep !== null &&
            developmentProcess?.[currentStep] &&
            title === "Mobile App Development Services" && (
                ...
                <div className="w-[96%] h-[93%] rounded-[30px] sm:rounded-[48px] bg-white bg-opacity-10 flex justify-between items-center sm:py-0 py-2">
                  <div className="flex mx-auto w-[96%] items-center h-full">
                    ...
                    <div className=" text-white p-4 h-[100%] rounded-[25px] sm:mt-12 tracking-tighter flex flex-col gap-1 sm:gap-5">
                      <Image
                        src={developmentProcess[currentStep]?.image}
                        alt="icons"
                      />
      ...
      {/* ux design process */}
      {title === "Top UI/UX Design Services" && <UxDesignSvg />}
      {/* tools used  */}
